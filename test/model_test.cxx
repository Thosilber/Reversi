...
TEST_CASE("My Game Test")
{
    ...
    //access.set_player({1,4}, Player::dark);

    // Use Test_access to set `model.next_moves_` to allow the particular
    // (illegal) move that we'd like to try:
    access.next_moves()[{0, 3}] = Position_set {{0, 3}, {1,2}, {2,1}};
    ...
    // Check that turn goes to light
    CHECK(model.turn() == Player::light);
    // Check that the tiles {1,2} and {2,1} are now dark
    CHECK(model[{0, 3}] == Player::dark);
    CHECK(model[{1, 2}] == Player::dark);
    CHECK(model[{2, 1}] == Player::dark);

    model.play_move({2, 0});
    CHECK(model.turn() == Player::dark);
    CHECK(model[{2, 1}] == Player::light);
    ...
    model.play_move({0,2});
    CHECK(model.turn() == Player::dark);
    CHECK(model[{1, 1}] == Player::light);
    CHECK(model[{2, 2}] == Player::dark);
    ...
    CHECK(model.turn() == Player::dark);
    CHECK(model[{1, 2}] == Player::light);
    CHECK(model[{2, 2}] == Player::light);

    model.play_move({0,0});
    CHECK(model.turn() == Player::dark); // turn goes back to dark
    CHECK(model[{0, 1}] == Player::dark);
    CHECK(model[{0, 2}] == Player::dark);

    model.play_move({3,3});
    CHECK(model.turn() == Player::neither); // game ends
    CHECK(model[{2, 2}] == Player::dark);
    CHECK(model[{2, 3}] == Player::dark);
    CHECK(model[{3, 2}] == Player::dark);

    CHECK(model.winner() == Player::dark); // game over, check winner
